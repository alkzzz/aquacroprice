...
class Rice(gym.Env):
    ...
    def step(self, action):
        ...
        if self.day_counter >= self.days_to_irr:
            self.day_counter = 0
            depth = self.action_depths[int(action)]  # Irrigate based on the action
        else:
            depth = 0  # No irrigation if 7 days have not passed
        ...
        # Log the applied irrigation depth
        ...
        if terminated:
            ...
            # Reset the first irrigation flag for the next episode
