...
train_env = DummyVecEnv([lambda: Monitor(Maize(mode='train', year1=1982, year2=2002), './monitor_logs')])

# Custom reward logging callback
reward_logging_callback = RewardLoggingCallback(experiment)

# Training parameters
train_timesteps = 250000  # Adjust the number of timesteps as needed
...
plt.bar(agents, mean_rewards, yerr=std_rewards, capsize=10, color=['green', 'red', 'blue'])
plt.ylabel('Mean Reward')
plt.title('Comparison of Total Rewards (PPO vs DQN vs Random Agent)')
plt.grid(True)
...
plt.bar(agents, mean_yields, capsize=10, color=['green', 'red', 'blue'])
plt.ylabel('Mean Yield (tonne/ha)')
plt.title('Comparison of Mean Yields (PPO vs DQN vs Random Agent)')
plt.grid(True)
...
plt.bar(agents, mean_irrigations, capsize=10, color=['green', 'red', 'blue'])
plt.ylabel('Mean Irrigation (mm)')
plt.title('Comparison of Mean Irrigation (PPO vs DQN vs Random Agent)')
plt.grid(True)
